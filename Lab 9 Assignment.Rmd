...
OMEGAvaldata = valData[,1:2]
OMEGAvaldata$OMEGAraster = extract(OMEGAraster, newValcovsXy)[,1]
view(OMEGAvaldata)
colnames(OMEGAvaldata)[3] = 'OMEGA raster'


# discrimination stats for all models

OMEGAvaldata = data.frame(matrix(nrow=0, ncol=9))
OMEGAvaldata = ncol(OMEGAvaldata)-2

for(i in 1:OMEGAvaldata){
  ...
  #Put them all together and save the values
  summaryI = c(i, auc$AUC, corr, ll, kappaOpt[[2]], sens$sensitivity, spec$specificity, tss, kappa[[1]])
  OMEGAsummaryEval = rbind(OMEGAsummaryEval, summaryI)
}

OMEGAsummaryEval = OMEGAsummaryEval %>% 
  setNames(c('model', 'AUC', 'corr', 'll', 'threshold', 'sens', 'spec', 'tss', 'kappa'))
