...
head(OMEGAvaldata)
```
The AUC value for the ensemble(5)model is the highest(higher values indicate better discrimination between classes and model performanc), and the kappa is closest to 1(better agreement. Based on these metrics, the Ensemble model is better than the others.
...
```{r}
 ...
 #(A)Fit a GLM using presence-background data (LAB8/9)       
    ...
    summary(glmModelBack)
      ...
      plot(glmMapBack, main="Background data")
...
#(B)Fit a GLM using presence-absence data
  glmModelAbs = glm(pres ~ canopy + elev + I(elev^2) + mesic1km + precip, family='binomial', data=presAbsCovs)
    summary(glmModelAbs)
      ...
      plot(glmMapAbs, main="absence data")
...
NEWval = data.frame('ID' = 1:nrow(tmpA)) %>% 
  ...
  setNames(c('AUC','kappaOpt','tss', 'kappa'))}
summaryEval2
head(NEWval)
```
It appears that the Absence data has a slightly smaller AUC than background, but does better in all other metrics; however, kappa for neither is very good. I would say that the glm based on absence data is better, since having that data will be more accurate than simulating data.
